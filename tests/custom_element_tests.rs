...
use website_base::{
    ...
    document::{create_element, create_element_with_text, HtmlNode},
};
...
fn test_define_custom_element_invalid_name() {
    ...
    let definition = CustomElementDefinition {
        tag_name: "invalidelement".to_string(),
    };
...
fn test_define_custom_element_duplicate_name() {
    // Create a custom element constructor
    let definition = CustomElementDefinition {
        tag_name: "test-duplicate-1".to_string(),
    };
...
fn test_define_custom_rust_type() {
    ...
    let element = create_element(&definition.tag_name).unwrap();
...
fn test_get_custom_element_not_registered() {
    ...
    assert!(
        ...
        "Expected None for unregistered element"
    );
}
...
fn test_get_custom_element_after_registration() {
    let definition = CustomElementDefinition {
        tag_name: "get-test-element".to_string(),
    };

    // First, check that it doesn't exist
    let before_result = get_custom_element(&definition.tag_name);
    assert!(
        before_result.is_ok(),
        "Expected Ok result before registration"
    );
    assert!(
        before_result.unwrap().is_none(),
        "Expected None before registration"
    );
    ...
    let after_result = get_custom_element(&definition.tag_name);
    assert!(
        after_result.is_ok(),
        "Expected Ok result after registration"
    );
    assert!(
        after_result.unwrap().is_some(),
        "Expected Some after registration"
    );
}
...
fn test_get_custom_element_multiple_elements() {
    let definition1 = CustomElementDefinition {
        tag_name: "multi-test-element-1".to_string(),
    };

    let definition2 = CustomElementDefinition {
        tag_name: "multi-test-element-2".to_string(),
    };
    ...
    let result1 = get_custom_element(&definition1.tag_name).unwrap();
    assert!(result1.is_some(), "Expected first element to be registered");

    // Check second element doesn't exist yet
    let result2_before = get_custom_element(&definition2.tag_name).unwrap();
    assert!(
        result2_before.is_none(),
        "Expected second element to not be registered yet"
    );
    ...
    let result1_after = get_custom_element(&definition1.tag_name).unwrap();
    let result2_after = get_custom_element(&definition2.tag_name).unwrap();
    assert!(
        result1_after.is_some(),
        "Expected first element to still be registered"
    );
    assert!(
        result2_after.is_some(),
        "Expected second element to be registered"
    );
}
...
fn test_get_custom_element_conditional_registration() {
    ...
    let definition = CustomElementDefinition {
        tag_name: "conditional-test-element".to_string(),
    };

    // Check if element exists before registering
    if get_custom_element(&definition.tag_name).unwrap().is_none() {
    ...
    let final_result = get_custom_element(&definition.tag_name).unwrap();
    ...
    // Should still exist
    let final_result2 = get_custom_element(&definition.tag_name).unwrap();
    assert!(
        final_result2.is_some(),
        "Expected element to still be registered"
    );
}
...
fn test_define_custom_rust_type_2() {
    let definition = CustomElementDefinition {
        tag_name: "bstruct-table-2".to_string(),
    };
    ...
    let element = create_element_with_text(&definition.tag_name, Some("Hello from bstruct-table-2"))
        .unwrap()
        ;
